...
public:
	...
	Range(const T& _first, const T& _last, const T& _step = 1) : first(_first), last(_last), step(_step) {
		if ((first < last && step < 0) || (first > last1 && step > 0) || step == 0) {
	...
	Range_iterator<T> begin() const{
		return Range_iterator(first, last, step, 0);
	}
	Range_iterator<T> end() const{
		return Range_iterator(first, last, step, last + step);
	}
	//operator++() {}
};
...
public:
	...
	bool operator!=(Range_iterator& other) return index != other.index;
	T operator*() const 
